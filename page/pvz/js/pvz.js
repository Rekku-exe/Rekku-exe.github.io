...
function cliqueslot(x, y){
    if(tabPlants[x][y] == 0){
        ...
        if(tabPlants[x][y].type == 'sun' && tabPlants[x][y].cooldown == 0){
            money += 25;
            document.getElementById("money").innerHTML = money;
            document.getElementById('slot-'+x+'-'+y).innerHTML = "";
            tabPlants[x][y].cooldown = tabPlants[x][y].maxCooldown;
        }
    }
}
...
function clickSun(){
    money += 25;
    document.getElementById("money").innerHTML = money;
    document.getElementById('theSun').remove();
}
...
(function () {
    function main( tFrame ) {
                ...
                if(tabPlants[y][x] != 0){
                    ...
                    switch(tabPlants[y][x].type){
                        ...
                        case 'sun':
                            ...
                            if(tabPlants[y][x].cooldown == 1){
                                document.getElementById('slot-'+y+'-'+x).innerHTML = "<img class='sun entity' src='./img/sun.webp'/>";
                            }
                        ;
                    }
                }
        ...
        if(freesun > 0){
            ...
            document.getElementById('terrain').innerHTML += "<img id='theSun' style='margin-top:"+(Math.floor(Math.random() * 400)+50)+"px; margin-left:"+(Math.floor(Math.random() * 900)+50)+"px;' src='./img/sun.webp' onclick='clickSun()'/>";
            freesun = 5000;
        }
    }
    main();
